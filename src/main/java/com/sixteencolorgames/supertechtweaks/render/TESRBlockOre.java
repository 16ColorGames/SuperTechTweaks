        ...
        try {
            ...
            for (int i = 0; i < 7; i++) {//for each of the ores inside the block
                ...
                if (metal != 0) {
                    ...
                    int color = met.getColor();//decode the color from the Ores enum
                    GlStateManager.color(((float) ((color >> 16) & 0xFF)) / 255, ((float) ((color >> 8) & 0xFF)) / 255,
                            ((float) (color & 0xFF)) / 255);//set the render color
