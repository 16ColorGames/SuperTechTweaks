...
public class ResearchToast implements IToast {
	...
	public IToast.Visibility draw(GuiToast toastGui, long delta) {
		...
		DisplayInfo displayinfo = this.research.getDisplay();
		...
		if (displayinfo != null) {
			...
			if (list.size() == 1) {
				toastGui.getMinecraft().fontRenderer.drawString(
						I18n.format("researchs.toast." + displayinfo.getFrame().getName()), 30, 7, i | -16777216);
				...
				if (delta < 1500L) {
					int k = MathHelper
							...
							| 67108864;
					toastGui.getMinecraft().fontRenderer.drawString(
							I18n.format("researchs.toast." + displayinfo.getFrame().getName()), 30, 11, i | k);
				} else {
					int i1 = MathHelper
