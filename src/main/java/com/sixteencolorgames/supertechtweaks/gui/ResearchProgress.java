...
public class ResearchProgress implements Comparable<ResearchProgress> {
	private final Map<String, CriterionProgress> criteria = Maps.<String, CriterionProgress> newHashMap();
	private String[][] requirements = new String[0][];

	/**
	 * Update this ResearchProgress' criteria and requirements
	 */
	...
	public String toString() {
		return "ResearchProgress{criteria=" + this.criteria + ", requirements=" + Arrays.deepToString(this.requirements)
				+ '}';
	}
	...
	public static ResearchProgress fromNetwork(PacketBuffer p_192100_0_) {
		ResearchProgress researchprogress = new ResearchProgress();
		int i = p_192100_0_.readVarInt();

		for (int j = 0; j < i; ++j) {
			researchprogress.criteria.put(p_192100_0_.readString(32767),
					CriterionProgress.read(p_192100_0_, researchprogress));
		}
	...
	public int compareTo(ResearchProgress p_compareTo_1_) {
	...
	public static class Serializer implements JsonDeserializer<ResearchProgress>, JsonSerializer<ResearchProgress> {
		public JsonElement serialize(ResearchProgress p_serialize_1_, Type p_serialize_2_,
		...
		public ResearchProgress deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_,
				JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
			JsonObject jsonobject = JsonUtils.getJsonObject(p_deserialize_1_, "research");
			JsonObject jsonobject1 = JsonUtils.getJsonObject(jsonobject, "criteria", new JsonObject());
			ResearchProgress researchprogress = new ResearchProgress();

			for (Entry<String, JsonElement> entry : jsonobject1.entrySet()) {
				String s = entry.getKey();
				researchprogress.criteria.put(s,
						CriterionProgress.fromDateTime(researchprogress, JsonUtils.getString(entry.getValue(), s)));
			}
