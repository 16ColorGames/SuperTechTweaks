...
public class SimplexNoise { // Simplex noise in 2D, 3D and 4D
	...
	public double get3dNoiseValue(int x, int y, int z, Vec3d offset, double scale) {
		double noiseX = Math.abs((x + offset.x) * scale);
		double noiseY = Math.abs((y + offset.y) * scale);
		double noiseZ = Math.abs((z + offset.z) * scale);

		return noise(noiseX, noiseY, noiseZ);
	}

	public double get2dNoiseValue(int x, int z, Vec3d offset, double scale) {
		double noiseX = Math.abs((x + offset.x) * scale);
		double noiseZ = Math.abs((z + offset.z) * scale);

		return noise(noiseX, noiseZ);
	}
}