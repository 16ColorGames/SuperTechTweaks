...
public class BakedModelInserter implements IBakedModel {
	private static IModel modelInserterFlat;
	...
	public BakedModelInserter() {
		try {
			modelInserterFlat = ModelLoaderRegistry
					.getModel(new ResourceLocation(SuperTechTweaksMod.MODID, "block/inserter"));
		}
	...
	private IBakedModel getActualModel(ArrayList<Boolean> state, EnumFacing facing) {
		...
		if (cache.containsKey(cacheKey))
			...
			IModel modelInserter = modelInserterFlat;

			bakedModel = modelInserter.bake(new TRSRTransformation(facing), format, textureGetter::apply);
	...
	public List<BakedQuad> getQuads(@Nullable IBlockState blockState, @Nullable EnumFacing side, long rand) {
		...
		if (blockState instanceof IExtendedBlockState) {
			...
			if (extendedBlockState.getUnlistedNames().contains(BlockInserter.FACING))
				facing = extendedBlockState.getValue(BlockInserter.FACING);
		}
	...
	public ItemOverrideList getOverrides() {
		return InserterItemOverrideList.INSTANCE;
	}

	private static class InserterItemOverrideList extends ItemOverrideList {
		static InserterItemOverrideList INSTANCE = new InserterItemOverrideList();

		InserterItemOverrideList() {
