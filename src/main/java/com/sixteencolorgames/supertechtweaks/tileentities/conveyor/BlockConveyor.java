...
/**
 * Big thanks to
 * https://minecraft.curseforge.com/projects/industrial-conveyor-belts?gameCategorySlug=mc-mods&projectID=265780
 * for inspiration and starting code
 * 
 * @author oa10712
 *
 */
public class BlockConveyor extends BlockConveyorBase implements ITileEntityProvider {
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		if (worldIn.getTileEntity(pos) != null && worldIn.getTileEntity(pos) instanceof TileEntityConveyor) {
			...
			if (worldIn.getTileEntity(posFrontUp) != null
					&& worldIn.getTileEntity(posFrontUp) instanceof TileEntityConveyorBase)
				slopeUp = ((TileEntityConveyorBase) worldIn.getTileEntity(posFrontUp)).getFacing() == facing;

			if (worldIn.getTileEntity(posBackUp) != null
					&& worldIn.getTileEntity(posBackUp) instanceof TileEntityConveyorBase)
			...
			if (worldIn.getTileEntity(posLeft) != null
					&& worldIn.getTileEntity(posLeft) instanceof TileEntityConveyorBase)
				turnLeft = ((TileEntityConveyorBase) worldIn.getTileEntity(posLeft)).getFacing() == facing.rotateY();

			if (worldIn.getTileEntity(posRight) != null
					&& worldIn.getTileEntity(posRight) instanceof TileEntityConveyorBase)
				turnRight = ((TileEntityConveyorBase) worldIn.getTileEntity(posRight)).getFacing() == facing
						.rotateYCCW();
			...
			if (worldIn.getTileEntity(posFrontDown) != null
					&& worldIn.getTileEntity(posFrontDown) instanceof TileEntityConveyor)
				conveyorFrontDown = (TileEntityConveyor) worldIn.getTileEntity(posFrontDown);

			if (worldIn.getTileEntity(posBackDown) != null
					&& worldIn.getTileEntity(posBackDown) instanceof TileEntityConveyor)
			...
			if (worldIn.getTileEntity(posBackRight) != null
					&& worldIn.getTileEntity(posBackRight) instanceof TileEntityConveyor)
				conveyorBackRight = (TileEntityConveyor) worldIn.getTileEntity(posBackRight);

			if (worldIn.getTileEntity(posBackLeft) != null
					&& worldIn.getTileEntity(posBackLeft) instanceof TileEntityConveyor)
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		...
		if (!worldIn.isRemote) {
			...
			if (tileConveyor != null && !worldIn.getBlockState(fromPos).isOpaqueCube()) {
		...
		if (validEntity(entity)) {
			if (tileConveyorBase instanceof TileEntityConveyor) {
				...
				if (tileConveyor.isSlopeUp()) {
					...
					thiss = new Vec3d(thiss.x, next.y, thiss.z);
					...
					if (diffY > .9) {
						...
						if (diffXZ < .9)
							entity.setPosition(entity.posX + .15 * facing.getFrontOffsetX(), entity.posY,
									entity.posZ + .15 * facing.getFrontOffsetZ());
					}
				}
	...
	protected BlockStateContainer createBlockState() {
		...
		IUnlistedProperty[] unlistedProperties = new IUnlistedProperty[] { FACING, SLOPE_UP, SLOPE_DOWN, TURN_LEFT,
				TURN_RIGHT, POWERED };
